        ...
        options =>
        {
            ...
            options.ConnectionString =
                ...
                ?? throw new InvalidOperationException(
                    "PostgresConnection is not configured.");
        });
});
...
app.MapGet(
    "/api/v1/meta/selfcheck",
    (
        [FromServices]
        AIOps.Abstractions.Agents.IAgentGateway gateway,

        [FromServices]
        AIOps.Abstractions.Audit.IAuditStore audit,

        [FromServices]
        AIOps.Abstractions.Persistence.ITicketRepository tickets,

        [FromServices]
        AIOps.Abstractions.Time.IClock clock) =>
        Results.Ok(
            new
            {
                ...
                orleans =
                    "co-hosted silo, PostgreSQL ADO.NET grain storage (ticketStore)"
            }))
    .WithTags("Meta")
    .AllowAnonymous();
...
app.MapGet(
    ...
    (IConfiguration config) =>
        Results.Ok(
            ...
            {
                agentGateway =
                    config["AI:AgentGatewayMode"] ?? "Fake",

                nimBaseUrl =
                    config["AI:NvidiaNim:BaseUrl"],

                nimModel =
                    config["AI:NvidiaNim:Model"],

                nimApiKeyConfigured =
                    !string.IsNullOrEmpty(
                        config["AI:NvidiaNim:ApiKey"])
            }))
    .WithTags("Meta")
    .AllowAnonymous();
